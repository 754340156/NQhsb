...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    for (UIView *view in cell.subviews) {
        if ([view isKindOfClass:[UILabel class]]) {
            ...
            if ([label.text isEqualToString: @"未完成"]) {
    ...
    label.hidden = ![model.isShelves boolValue];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([analysic.isShelves isEqualToString:@"0"]) {
        HWMusicquestionReportFinishVC *job = [[HWMusicquestionReportFinishVC alloc] init];
...
-(void)pushtoModleVC
{
    HWMusicSelectedHintVC  *Videoselected=[[HWMusicSelectedHintVC alloc] init];
...
{
    HWMusicAnalysicModel *model=_dataListArray[indexPath.row];
    [_dataListArray removeObjectAtIndex:indexPath.row];
    NSDictionary *parameters=@{@"account":[UserInfo account].account,
                               @"token":[UserInfo account].token,
                               @"dataId":model.dataId,
                               @"token":[UserInfo account].token};
