...
- (void)HWAddStudyPlanCellDelegate_addStudyPlanOrRemoveWithBtn:(UIButton *)sender Model:(HWCollectionModel *)model Cell:(HWAddStudyPlanCell *)cell
{
    ...
    if (sender.selected) {
        ...
        [dic setObject:model forKey:[NSString stringWithFormat:@"%ld-%ld-%ld",(long)self.currentIndex,(long)indexpath.section,(long)indexpath.row]];
        [self.addDataArray addObject:dic];
    }else
    {
        
        for (NSInteger i = 0; i < self.addDataArray.count; i++) {
            NSDictionary *dic = self.addDataArray[i];
            if (dic[[NSString stringWithFormat:@"%ld-%ld-%ld",(long)self.currentIndex,(long)indexpath.section,(long)indexpath.row]]) [self.addDataArray removeObject:dic];
        }
        //移除数组
    }
}
