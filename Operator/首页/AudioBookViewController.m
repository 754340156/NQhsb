...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    switch (indexPath.section) {
        ...
        {
            if (indexPath.row == 0) {
                [self pushVc:@"5" api:checklist IsMy:NO];
            }
        }
            break;
        case 1:
        {
            if (indexPath.row == 0) {
                 [self pushVc:@"5" api:listOfMe IsMy:YES];
            }else{
