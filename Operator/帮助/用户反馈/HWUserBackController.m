...
- (void)setRightButton
{
    ...
    self.collectionView.backgroundColor = BXT_BACKGROUND_COLOR;
...
- (void)submitAction
{
    ...
    if ([NSString StringIsNULL:self.remarkTV.text] &&  self.selectedPhotos.count==0)
    {
        [self showHint:@"请输入备注信息或添加图片"];
        return;
    }else{
...
//                NSString *pic = [names componentsJoinedByString:@","];
//                [self netWorkHelp:self.remarkTV.text photps:pic];
//            }else{
//                [self showHint:@"图片服务器出错，请联系客服"];
//            }
//            [self hideHud];
//        }];
        ...
        for (UIImage *image in _selectedPhotos) {
            NSString *imageUrl = [ALiYunTool upLoadImage:image];
            [photoArr addObject:imageUrl];
        }
        [self hideHud];
        NSString *picStr = [photoArr componentsJoinedByString:@","];
        [self netWorkHelp:self.remarkTV.text photps:picStr];
        
    }
    //发请求
}
-(void)netWorkHelp:(NSString *)content photps:(NSString *)photos
{
    NSDictionary *dic = @{@"account":[UserInfo account].account,
                          @"token":[UserInfo account].token,
                          @"context":content,
                          @"pic":photos};
    [NetWorkHelp netWorkWithURLString:@"/user/addFeedback.do"
                           parameters:dic
