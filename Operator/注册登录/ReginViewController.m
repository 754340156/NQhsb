...
- (void)viewDidLoad {
    ...
    [self.kBxtReginBtn setBackgroundColor:KTabBarColor];
...
-(NSString *)navTitle
{
    ...
    if ([_selectType isEqualToString:@"1"]) {
        title = @"忘记密码";
        [self.kBxtReginBtn setTitle:@"提交" forState:UIControlStateNormal];
    }else{
        title = @"注册";
        [self.kBxtReginBtn setTitle:@"注册" forState:UIControlStateNormal];
    }
    return title;
}
...
-(void)passwdNetWorkPhont:(NSString *)phone verCode:(NSString *)verCode passwd:(NSString *)passwd passwdTwo:(NSString *)passwdTwo
{
    NSDictionary *dic = @{@"phone":phone,
                          @"verCode":verCode,
                          @"password":passwd,
                          @"password2":passwdTwo};
    [NetWorkHelp netWorkWithURLString:forgetPassword
                           parameters:dic
                         SuccessBlock:^(NSDictionary *dic) {
                             if ([dic[@"code"] intValue] == 0) {
                                 [self showHint:@"修改成功"];
...
-(void)kBxtReginBtnClick
{
    if (GJCFStringIsMobilePhone(self.kBxtPhoneTF.text) && self.kBxtAuthTF.text) {
        if (![NSString judgePassWordLegal:_kBxtPasswdTF.text]) {
        ...
        if ([self.titleLabel.text isEqualToString: @"忘记密码"]) {
            [self passwdNetWorkPhont:_kBxtPhoneTF.text
                             verCode:_kBxtAuthTF.text
                              passwd:_kBxtPasswdTF.text
                           passwdTwo:_kBxtAffirmTF.text];
        }else{
