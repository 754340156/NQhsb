...
#define UMSocial_Item_Count_PerRow 1 //列数 （计算高度时会用到）
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        CGFloat UMSocial_Item_Width = 80;
        if ([UIScreen mainScreen].bounds.size.width > [UIScreen mainScreen].bounds.size.height) {
            UMSocial_Item_Width = [UIScreen mainScreen].bounds.size.height/375 * 80;
        }else{
            UMSocial_Item_Width = [UIScreen mainScreen].bounds.size.width/375 * 80;
        }
...
//    selfFrame.size.width = superView.frame.size.width;
...
- (NSMutableDictionary *)dictWithPlatformName:(NSNumber *)platformType
{
    ...
    switch (platformType_int) {
        ...
        case UMSocialPlatformType_WechatSession:
            imageName = @"ICON_shareit_WX";
            platformName = UMLocalizedString(@"ICON_shareit_WX",@"微信");
            break;
        case UMSocialPlatformType_WechatTimeLine:
            imageName = @"ICON_shareit_PYQ";
        ...
        case UMSocialPlatformType_QQ:
            imageName = @"ICON_shareit_QQ";
            platformName = UMLocalizedString(@"ICON_shareit_QQ",@"QQ");
            break;
        case UMSocialPlatformType_Qzone:
            imageName = @"ICON_shareit_QQKJ";
            platformName = UMLocalizedString(@"ICON_shareit_QQKJ",@"QQ空间");
            break;
    ...
    [dict setObject:imageName forKey:kUMSSharePlatformIconName];
    [dict setObject:platformName forKey:kUMSplatformType];
    //为各平台创建按钮
    UMShareMenuItem *cell = [[UMShareMenuItem alloc] init];
    [cell reloadDataWithImage:[UIImage imageNamed:imageName] platformName:platformName];
