...
- (instancetype)initWithOrigin:(CGPoint)origin andHeight:(CGFloat)height {
    ...
    if (self) {
        ...
        _rightTableView.separatorColor = color_cccccc;
        ...
        //add bottom shadow
        _bottomShadow = [[UIView alloc] initWithFrame:CGRectMake(0, self.frame.size.height, screenSize.width,0)];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (_rightTableView==tableView) {
        ...
        CGFloat textLayerInterval = self.frame.size.width / ( _numOfMenu * 2);
        CGPoint titlePosition = CGPointMake( (indexPath.row * 2 + 1) * textLayerInterval , self.frame.size.height / 2);
        CATextLayer *title;
        NSString *titleString = [_dataSource menu:self titleForColumn:indexPath.row];
        title = [self createTextLayerWithNSString:titleString withColor:KTabBarColor andPosition:titlePosition];
        UILabel *viceLabel = (UILabel *)[self viewWithTag:indexPath.row+2000];
        [viceLabel.layer addSublayer:title];
    }

    
    
    if (self.delegate || [self.delegate respondsToSelector:@selector(menu:didSelectRowAtIndexPath:Block:)]) {
        
        BOOL haveRightTableView = [_dataSource haveRightTableViewInColumn:_currentSelectedMenudIndex];
        
        __block BOOL iscount = YES;
        
        [self.delegate menu:self didSelectRowAtIndexPath:[JSIndexPath indexPathWithCol:self.currentSelectedMenudIndex leftOrRight:leftOrRight leftRow:_leftSelectedRow row:indexPath.row] Block:^(NSInteger count) {
            if (count) {
                iscount = NO;
            }
        }];
        
        if (((leftOrRight==0 && !haveRightTableView) || iscount) || leftOrRight==1) {
...
-(void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    if (self.delegate || [self.delegate respondsToSelector:@selector(menu:didSelectRowAtIndexPath:Block:)]) {
        ...
        [self.delegate menu:self didSelectRowAtIndexPath:[JSIndexPath indexPathWithCol:self.currentSelectedMenudIndex leftOrRight:-1 leftRow:-1 row:indexPath.row] Block:^(NSInteger count) {
