...
@interface HWAddPicController ()<UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout,TZImagePickerControllerDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate>
...
- (void)saveAction
{
    ...
    if ([NSString  StringIsNULL:_titleTF.text])
    {
        [self showHint:@"请输入标题"];
        return;
    }else if ([NSString StringIsNULL:_remarkTV.text])
    {
        [self showHint:@"请输入备注信息"];
        return;
    }else if (_selectedPhotos.count==0)
    {
